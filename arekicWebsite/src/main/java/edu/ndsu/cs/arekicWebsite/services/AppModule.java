...
import edu.ndsu.cs.arekicWebsite.entities.implementations.LocalSecurityRealm;
...
public class AppModule
{
	...
	{
		configuration.add("users", factory.createChain("/users/**").add(factory.authc()).build());
	}

	@Contribute(WebSecurityManager.class)
	public static void addRealm(Configuration<Realm> configuration, @Inject UserAccountService uas)
	{
		LocalSecurityRealm localSecurityRealm = new LocalSecurityRealm(uas);
		localSecurityRealm.setCredentialsMatcher(new HashedCredentialsMatcher("SHA-512"));
	        	   	
		configuration.add(localSecurityRealm);
	}

	public static UserAccountService buildUserAccountService(@Inject CayenneService cayenneService)
	{
		return new CayenneUserAccountServiceImpl(cayenneService);
	}

	public static BookDatabaseService buildDatabaseService(CayenneService cayenneService)
	{
		return new CayenneBookDatabaseServiceImpl(cayenneService);
	}

    public static void bind(ServiceBinder binder)
    {
    	//binder.bind(BookDatabaseService.class, MockDatabase.class);
